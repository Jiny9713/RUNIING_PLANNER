...
Streamlit 기반 마라톤 주간 훈련 플래너입니다. 최신 기본 엔진(`planner_core.py`)은 Coach.md 철학을 따르면서 `injury_flag`를 통한 **부상/질병 기반 주간 볼륨 휴리스틱**을 제공합니다. `app_streamlit.py`에서는 사이드바의 “지난주 거리 상태” 라디오 버튼으로 해당 값을 설정할 수 있습니다. 안전 스위치들은 여전히 자동 로직으로 적용되지 않으므로, 러너는 Coach.md의 체크리스트를 참고해 수동 보정을 해야 합니다.

## 폴더 구조
```
Running_Planner/
├── planner_core.py             # 기본 엔진 (injury-aware 볼륨)
├── planner_core_v1_0.py        # v1.0 엔진 (injury flag 없음)
├── planner_core_v1_1.py        # v1.0 엔진 기반 injury-aware 휴리스틱 실험본
├── app_streamlit.py            # 기본 Streamlit UI (injury flag 입력 포함)
├── app_streamlit_v1_0.py       # v1.0 전용 UI
├── app_streamlit_v1_1.py       # planner_core_v1_1 전용 UI
├── tests/
│   ├── test_planner_core.py    # 기본 동작 & injury heuristics 테스트
...
```
브라우저가 열리면 사이드바 입력을 채운 뒤 **주간 플랜 생성** 버튼을 눌러 플랜을 확인합니다. 기본 UI에는 “지난주 거리 상태” 라디오가 추가되어 `injury_flag`를 설정할 수 있습니다.
