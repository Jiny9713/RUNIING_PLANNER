...
```
Running_Planner/
├── planner_core.py          # PlanConfig, Planner, generate_week_plan을 포함한 핵심 엔진 (v1.0)
├── planner_core_v1_1.py     # 부상 여부를 반영한 주간 볼륨 휴리스틱 (v1.1)
├── app_streamlit.py         # Streamlit UI (v1.0, 기본 엔진)
├── app_streamlit_v1_1.py    # Streamlit UI (v1.1, injury flag 입력 포함)
├── tests/
...
v1.1 엔진(`planner_core_v1_1.py`)을 사용할 경우, 추가적으로 `injury_flag`(지난주 감소 원인이 부상/질병인지 여부)를 입력받아 주간 볼륨 조정에 반영합니다.
...
pytest
```
`tests/test_planner_core.py`에는 v1.0 시나리오(Goal Mode/Stage/Taper)가, `tests/test_planner_core_v1_1.py`에는 부상 여부에 따른 볼륨 조정 시나리오가 포함되어 있습니다.
